...
NvEncodeAPIGetMaxSupportedVersion_Type pNvEncodeAPIGetMaxSupportedVersion = nullptr;
NvEncodeAPICreateInstance_Type pNvEncodeAPICreateInstance = nullptr;

int LoadDll() {
    if (g_hModule == nullptr) {
#if defined(_WIN64)
        g_hModule = LoadLibrary(TEXT("nvEncodeAPI64.dll"));
#elif defined(_WIN32)
        g_hModule = LoadLibrary(TEXT("nvEncodeAPI.dll"));
#endif
    }
    if (g_hModule == nullptr) {
        LOG(ERROR) << "[nvenc] Error: NVENC library file is not found. Please ensure NV driver is installed.";
        return false;
    }

    if (pNvEncodeAPIGetMaxSupportedVersion == nullptr) {
#if defined(_WIN32)
        pNvEncodeAPIGetMaxSupportedVersion = (NvEncodeAPIGetMaxSupportedVersion_Type)GetProcAddress(g_hModule, "NvEncodeAPIGetMaxSupportedVersion");
#else
        pNvEncodeAPIGetMaxSupportedVersion = (NvEncodeAPIGetMaxSupportedVersion_Type)dlsym(hModule, "NvEncodeAPIGetMaxSupportedVersion");
#endif
    }
    if (pNvEncodeAPIGetMaxSupportedVersion == nullptr) {
        LOG(ERROR) << "[nvenc] Error: NVENC library not found function NvEncodeAPIGetMaxSupportedVersion.";
        return false;
    }

    if (pNvEncodeAPICreateInstance == nullptr) {
#if defined(_WIN32)
        pNvEncodeAPICreateInstance = (NvEncodeAPICreateInstance_Type)GetProcAddress(g_hModule, "NvEncodeAPICreateInstance");
#else
        pNvEncodeAPICreateInstance = (NvEncodeAPICreateInstance_Type)dlsym(hModule, "NvEncodeAPICreateInstance");
#endif
    }
    if (pNvEncodeAPICreateInstance == nullptr) {
        LOG(ERROR) << "[nvenc] Error: NVENC library not found function NvEncodeAPICreateInstance.";
        return false;
    }
    return true;
}
...
void FreeDll() {
    ...
    pNvEncodeAPIGetMaxSupportedVersion = nullptr;
    pNvEncodeAPICreateInstance = nullptr;
}
    ...
    m_hEncoder(nullptr)
{
    if (!LoadDll()) {
        FreeDll();
        NVENC_THROW_ERROR("NVENC library load fail", NV_ENC_ERR_INVALID_CALL);
        return;
    }
...
void NvEncoder::LoadNvEncApi()
{
    ...
    NVENC_API_CALL(pNvEncodeAPIGetMaxSupportedVersion(&version));
    ...
    NVENC_API_CALL(pNvEncodeAPICreateInstance(&m_nvenc));
}
