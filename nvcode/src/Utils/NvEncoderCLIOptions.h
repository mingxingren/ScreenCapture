...
public:
    ...
    std::string GetHelpMessage(bool bMeOnly = false, bool bUnbuffered = false, bool bHide444 = false, bool bOutputInVidMem = false)
    {
        ...
        oss << "-irperiod    Interval between successive intra refresh when -bf=0" << std::endl
            << "-ircnt       Length of intra refresh in number of frames for periodic intra refresh when -bf=0" << std::endl
            << "-ltr         Max number of LTR frames in DPB for LTR Per Picture mode" << std::endl
            << "-nonrefp     (No value) Enable automatic insertion of non-reference P-frames" << std::endl
            ;
...
public:
    ...
    virtual void SetInitParams(NV_ENC_INITIALIZE_PARAMS *pParams, NV_ENC_BUFFER_FORMAT eBufferFormat)
    {
        ...
        for (unsigned i = 0; i < tokens.size(); i++)
        {
            ...
            if (tokens[i] == "-irperiod" && ++i != tokens.size())
            {
                if (IsCodecH264())
                {
                    config.encodeCodecConfig.h264Config.enableIntraRefresh = 1;
                    ParseInt("-irperiod", tokens[i], &config.encodeCodecConfig.h264Config.intraRefreshPeriod);
                }
                else
                {
                    config.encodeCodecConfig.hevcConfig.enableIntraRefresh = 1;
                    ParseInt("-irperiod", tokens[i], &config.encodeCodecConfig.hevcConfig.intraRefreshPeriod);
                }
                continue;
            }
            if (tokens[i] == "-ircnt" && ++i != tokens.size())
            {
                if (IsCodecH264())
                {
                    ParseInt("-ircnt", tokens[i], &config.encodeCodecConfig.h264Config.intraRefreshCnt);
                }
                else
                {
                    ParseInt("-ircnt", tokens[i], &config.encodeCodecConfig.hevcConfig.intraRefreshCnt);
                }
                continue;
            }
            if (tokens[i] == "-ltr" && ++i != tokens.size())
            {
                if (IsCodecH264())
                {
                    config.encodeCodecConfig.h264Config.enableLTR = 1;
                    config.encodeCodecConfig.h264Config.ltrTrustMode = 0;
                    ParseInt("-ltr", tokens[i], &config.encodeCodecConfig.h264Config.ltrNumFrames);
                }
                else
                {
                    config.encodeCodecConfig.hevcConfig.enableLTR = 1;
                    config.encodeCodecConfig.hevcConfig.ltrTrustMode = 0;
                    ParseInt("-ltr", tokens[i], &config.encodeCodecConfig.hevcConfig.ltrNumFrames);
                }
                continue;
            }
            if (tokens[i] == "-nonrefp") {
